(() => {
  ...
  function initShowMoreBtns() {
    if (document.getElementsByClassName("comments__slider")) {
      ...
      for (let i = 0; i < slides.length; i++)
        if (slides[i].querySelector(".slide-comm__text").classList.contains("needShowMore")) slides[i].querySelector(".slide-comm__button").style.display = "block";
        else slides[i].querySelector(".slide-comm__button").style.display = "none";
    }
  }
  ...
  function animateCascadeSlide() {
    ...
    if (window.scrollY > cascadeSliderOffsetY && window.scrollY < cascadeSliderOffsetYBottom)
      for (let i = 0; i < cascadeSlidesCount; i++) {
        let cascadeSlideScale = 1 - (0.04 * (window.scrollY - cascadeSliderOffsetY + 56 - cascadeSlideHeight * i)) / cascadeSlideHeight;
        let cascadeSlideOpacity = 1 - (0.2 * (window.scrollY - cascadeSliderOffsetY + 56 - cascadeSlideHeight * i)) / cascadeSlideHeight;
